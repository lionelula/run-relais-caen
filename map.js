...
function showEndpointMarkers(coordinates) {
  ...
  coordinates.forEach((point, i) => {
    L.marker(point, { zIndexOffset: 1000, interactive: false, icon: L.divIcon({
      className: 'route-endpoint-icon', html: `<span class="route-endpoint ${i ? 'end' : ''}">${i ? 'B' : 'A'}</span>`,
...
function invalidateRoute() {
  ...
  showEndpointMarkers([routePoints.start.coordinates, routePoints.end.coordinates]);
}
...
async function calculateRoute() {
  ...
  const cacheKey = JSON.stringify([start, end]);
  ...
  try {
    WalkingRoutes.buildRequest(start, end);
    const route = routeCache.get(cacheKey) || await WalkingRoutes.requestRoute(start, end, controller.signal);
    ...
    showEndpointMarkers([route.points[0], route.points[route.points.length - 1]]);
    ...
    setRouteStatus('Trajet piéton calculé · suis la ligne verte.');
    fitRoute();
...
for (const key of ['start', 'end']) {
  ...
  document.querySelector(`#pick-${key}`).addEventListener('click', () => {
    ...
    document.querySelector(`#pick-${key}`).setAttribute('aria-pressed', 'true');
    document.querySelector('#cancel-pick').hidden = false;
    map.getContainer().classList.add('picking-route');
    setRouteStatus(`Clique sur la carte pour choisir ${key === 'start' ? 'le départ A' : 'l’arrivée B'}.`);
...
map.on('click', (event) => {
  ...
  routePoints[picking] = { id: null, coordinates: [point.lat, point.lng] };
  updateSelects();
  calculateRoute();
});
